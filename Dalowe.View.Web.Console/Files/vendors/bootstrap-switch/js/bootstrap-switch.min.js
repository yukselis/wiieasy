            ...
            init: function () {
                return this.each(function () {
                    ...
                    $element.find(inputSelector).on('change', function (e, skipOnChange) {
                        ...
                        if ($.isFunction(this.onBeforeChange))
                            this.onBeforeChange(e);

                        var currentRow = $element.closest('tr');
                        if ($(currentRow) != null && $(currentRow) != undefined && $(currentRow) != 'undefined' && $(currentRow).length > 0) {
                            var id = currentRow.data('id');
                            var type = currentRow.data('type');
                            if (id != null && id != 'undefined' && type != null && type != 'undefined') {
                                swal({
                                    title: "Durum bilgisini güncellemek istediğinize emin misiniz?",
                                    text: "Bu işlemi onaylarsanız seçili olan kayıt  " + (state ? "' Aktif" : "ve bu kayda bağlı tüm kayıtlar ' Pasif") + " ' duruma getirilecektir!",
                                    type: "warning",
                                    ...
                                    if (isConfirm) {
                                        ...
                                        $.Arwend.Ajax.GetResponse(function (response) {
                                            if (response.message.isNullOrEmpty()) {
                                                swal({ title: "İşlem Başarılı!", text: "Seçili kayıt başarıyla " + (state ? "Aktif" : "Pasif") + " duruma getirildi", type: "success", allowOutsideClick: true });

                                                e.preventDefault();
                                                $element.css('left', '');
                                                $this.val(thisState);
                                                //if (state != thisState) {
                                                    if (state) $element.removeClass('switch-off').addClass('switch-on'); else $element.removeClass('switch-on').addClass('switch-off');
                                                    if ($element.data('animated') !== false) $element.addClass("switch-animate");
                                                    if (typeof skipOnChange === 'boolean' && skipOnChange) return;
                                                    $element.parent().trigger('switch-change', { 'el': $this, 'value': state })
                                                //}
                                            }
                                            else{
